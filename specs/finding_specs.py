...
bases = [
    ...
    (CayleyPermutation((0, 1, 2)), CayleyPermutation((1, 3, 0, 2))),
    (CayleyPermutation((0, 2, 1)), CayleyPermutation((1, 2, 0, 3))),
    (
        CayleyPermutation((0, 2, 1)),
        CayleyPermutation((2, 1, 1, 0)),
    ...
    (CayleyPermutation((2, 0, 1)), CayleyPermutation((0, 0, 0, 1))),
    (CayleyPermutation((0, 2, 1)), CayleyPermutation((2, 1, 2, 0))),
    (CayleyPermutation((2, 0, 1)), CayleyPermutation((1, 2, 2, 0))),
    (
        CayleyPermutation((0, 2, 1)),
        CayleyPermutation((0, 0, 0)),
        CayleyPermutation((2, 1, 0, 1)),
    ),
    (
        CayleyPermutation((0, 2, 1)),
        CayleyPermutation((0, 0, 0)),
        CayleyPermutation((2, 0, 0, 1)),
    ),
    (CayleyPermutation((0, 2, 1)), CayleyPermutation((2, 1, 0, 0))),
    (
        CayleyPermutation((2, 0, 1)),
        CayleyPermutation((0, 0, 0)),
        CayleyPermutation((1, 0, 3, 2)),
    ),
]
...
part = 13
bases_to_run = [bases[part - 1]]  # can take a subset of the bases in the file
...
for basis in bases_to_run:
    ...
    for pack in all_packs:
        ...
        try:
            spec = searcher.auto_search(max_expansion_time=1800 * 4)  # set maxtime
            ...
            if spec_count == brute_force_count:
                ...
                with open(
                    f"{basis_desc}/no fusion/{Av(lex_min(basis))}_{pack.name}.json",
                    "w",
                ) as f:
                    f.write(json_spec)
            else:
...
# with open(f"summaries/wrong_counts_{basis_desc}_part_{part}.txt", "w") as f:
